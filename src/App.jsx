...
// ============================================
// By default, this uses localStorage (each device has its own data)
// For SHARED/COLLABORATIVE storage, see instructions at bottom of file
// ============================================
...
function DraggableActivity({ activity, index, onUpdate, onRemove, onDragStart, onDragOver, onDrop, isDragOver }) {
  return (
    ...
    >
      ...
      {activity.editedBy && <span style={{ fontSize: 11, color: '#888', background: '#f8f4ff', padding: '4px 8px', borderRadius: 6, whiteSpace: 'nowrap' }}>✏️ {activity.editedBy}</span>}
    ...
    <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }} onDragLeave={() => setDragOverIndex(null)}>
      ...
      {isOpen && (
        <div style={{ padding: '0 24px 24px', borderTop: '1px solid #f0f0f0' }}>
          <div style={{ fontSize: 12, color: '#888', padding: '12px 0 8px', display: 'flex', alignItems: 'center', gap: 6 }}><span>↕️</span> Drag items to reorder</div>
...
export default function App() {
  ...
  const addContact = () => { setData(p => ({ ...p, contacts: [...p.contacts, { id: Date.now(), name: '', phone: '', email: '', room: '', notes: '', addedBy: currentUser?.name }] })); addHistory('added contact'); };
  const removeContact = (id) => { setData(p => ({ ...p, contacts: p.contacts.filter(c => c.id !== id) })); addHistory('removed contact'); };
  const updateContact = (idx, field, val) => { setData(p => { const c = [...p.contacts]; c[idx] = { ...c[idx], [field]: val, lastEditedBy: currentUser?.name }; return { ...p, contacts: c }; }); };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #fdf6f9 0%, #fff 15%, #fff 85%, #f0e6ff 100%)', fontFamily: 'system-ui, sans-serif', color: '#4a4a6a' }}>
      ...
      {data.announcements.length > 0 && <div style={{ background: 'linear-gradient(90deg, #fff7e6, #fffbe6)', padding: '12px 24px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #ffe58f', position: 'relative', zIndex: 10 }}><span>📣</span><span style={{ flex: 1, color: '#d48806' }}>{data.announcements[0].text}</span><span style={{ color: '#888', fontSize: 13 }}>— {data.announcements[0].author}</span></div>}
      ...
      <main style={{ padding: 24, maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 10 }}>
        ...
        {activeTab === 'contacts' && <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><h2 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>👑 Family Members</h2><button onClick={addContact} style={btnPrimary}>+ Add Person</button></div>
          <p style={{ color: '#888', marginBottom: 24 }}>Everyone on the trip!</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
            {data.contacts.map((c, i) => <div key={c.id} style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}><span style={{ fontSize: 13, fontWeight: 600, color: '#764ba2' }}>#{i + 1}</span>{c.addedBy && <span style={{ fontSize: 11, color: '#888' }}>Added by {c.addedBy}</span>}<button onClick={() => removeContact(c.id)} style={{ width: 28, height: 28, borderRadius: 8, border: 'none', background: '#fff0f0', color: '#f5576c', fontSize: 16, cursor: 'pointer', marginLeft: 'auto' }}>×</button></div>
              {['name', 'phone', 'email', 'room'].map(f => <div key={f} style={{ marginBottom: 14 }}><label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: '#888', marginBottom: 6, textTransform: 'uppercase' }}>{f}</label><input type={f === 'email' ? 'email' : 'text'} value={c[f]} onChange={e => updateContact(i, f, e.target.value)} placeholder={f === 'room' ? 'Room assignment' : `Enter ${f}`} style={inputStyle} /></div>)}
              <div><label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: '#888', marginBottom: 6, textTransform: 'uppercase' }}>Notes</label><textarea value={c.notes} onChange={e => updateContact(i, 'notes', e.target.value)} placeholder="Dietary, medical..." style={textareaStyle} /></div>
            </div>)}
            {data.contacts.length === 0 && <div style={{ textAlign: 'center', padding: '48px 24px', color: '#888', gridColumn: '1/-1' }}><span style={{ fontSize: 48 }}>👨‍👩‍👧‍👦</span><p>No contacts yet. Click "Add Person"!</p></div>}
          </div>
        </div>}
        ...
        {activeTab === 'tips' && <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {data.recommendations.length === 0 && <div style={{ textAlign: 'center', padding: '48px 24px', color: '#888' }}><span style={{ fontSize: 48 }}>💭</span><p>No tips yet!</p></div>}
          </div>
        ...
        {activeTab === 'emergency' && <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20, marginBottom: 32 }}>
            {[{ k: 'hospital', i: '🏥', l: 'Nearest Hospital' }, { k: 'pharmacy', i: '💊', l: 'Pharmacy' }, { k: 'emergencyContact', i: '📞', l: 'Emergency Contact' }, { k: 'meetingSpot', i: '📍', l: 'Meeting Spot' }].map(f => <div key={f.k} style={cardStyle}><span style={{ fontSize: 32, display: 'block', marginBottom: 12 }}>{f.i}</span><label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: '#888', marginBottom: 6, textTransform: 'uppercase' }}>{f.l}</label><input type="text" value={data.emergency[f.k]} onChange={e => updateField(`emergency.${f.k}`, e.target.value, `updated ${f.l}`)} placeholder={`Enter ${f.l.toLowerCase()}`} style={inputStyle} /></div>)}
          </div>
          <div style={{ background: '#f8f4ff', borderRadius: 16, padding: 24 }}><h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 16 }}>🎒 Daily Checklist</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>{['☀️ Sunscreen & hats', '🔋 Chargers', '💧 Water bottles', '🍎 Snacks', '🎟️ Tickets', '👟 Comfy shoes', '☔ Ponchos', '🩹 First aid'].map((it, i) => <div key={i} style={{ background: '#fff', padding: '12px 16px', borderRadius: 10, fontSize: 14 }}>{it}</div>)}</div></div>
        </div>}
        ...
        {activeTab === 'history' && <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {(!data.editHistory || data.editHistory.length === 0) && <div style={{ textAlign: 'center', padding: '48px 24px', color: '#888' }}><span style={{ fontSize: 48 }}>📋</span><p>No activity yet!</p></div>}
          </div>
        </div>}
      </main>
...
function RecForm({ onAdd }) {
  ...
  const [cat, setCat] = useState('🍽️ Restaurant');
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: 20, marginBottom: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <select value={cat} onChange={e => setCat(e.target.value)} style={{ padding: '12px 16px', borderRadius: 10, border: '1px solid #e8e0f0', fontSize: 14, background: '#fff' }}>{['🍽️ Restaurant', '🎢 Ride', '🎭 Show', '🍦 Snack', '📸 Photo Spot', '🪄 Pro Tip'].map(c => <option key={c}>{c}</option>)}</select>
...
// ============================================
// FOR COLLABORATIVE/SHARED STORAGE:
// ============================================
// Replace the 'storage' object above with Firebase:
//
// 1. Go to firebase.google.com
// 2. Create a project
// 3. Go to Realtime Database and create one
// 4. Set rules to: { "rules": { ".read": true, ".write": true } }
// 5. Install: npm install firebase
// 6. Replace storage object with:
//
...
//     return snapshot.exists() ? { value: JSON.stringify(snapshot.val()) } : null;
//   },
//   async set(key, value) {
